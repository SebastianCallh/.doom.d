...
To reduce clutter the application specific configurations reside in their own config files.
- [[./app/sway.org][Sway]]
...
We use the following Python snippet to parse colors from ~#000000~ to ~0x000000~ format when it's needed (i.e. for [[file:app/alacritty.org][Alacritty]]).
